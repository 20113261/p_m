...
def dump_sql_and_upload():
    ...
    for each_table_name in table_names:
        ...
        os.system("mysqldump -h10.10.242.173 -u root -pshizuo0907 data_process {0} > {1}".
    ...
    for each_table_name in table_names:
        logger.debug("[start dump sql][host: 10.10.228.253][db: poi_merge][table name: {}]".format(each_table_name))
        os.system("mysqldump -h10.10.228.253 -umioji_admin -pmioji1109 poi_merge {0} > {1}".
                  format(each_table_name,
                         os.path.join(target_dir, each_table_name + '_data_process' + '.sql')))
        logger.debug("[dump sql finished][host: 10.10.228.253][db: poi_merge][table name: {}]".format(each_table_name))

    # mk base data
    for each_table_name in table_names:
        logger.debug("[start dump sql][host: 10.10.228.253][db: base_data][table name: {}]".format(each_table_name))
        os.system("cd {0};echo Y|mioji-mysqldump -h10.10.228.253 -umioji_admin -pmioji1109 base_data {1}".
                  format(target_dir, each_table_name))
        logger.debug("[dump sql finished][host: 10.10.228.253][db: base_data][table name: {}]".format(each_table_name))

    # data process load data
    for each_table_name in table_names:
        logger.debug("[start load sql][host: 10.10.242.173][table name: {}]".format(each_table_name))
        os.system("mysql -h10.10.242.173 -uroot -pshizuo0907 data_process < {1}".
                  format(each_table_name,
                         os.path.join(target_dir, each_table_name + '_data_process' + '.sql')))
        logger.debug("[load sql finished][host: 10.10.242.173][table name: {}]".format(each_table_name))
