    ...
    for line in db.QueryBySQL(sql):
        ...
        for raw_tag in split_pattern.split(json.loads(tagid)['daodao']):
            ...
            if tag in tag_dict:
                ...
                norm_tag_en_list.append(tag_dict[tag])
                continue
            if tag in key_words_dict:
                norm_tag_list.append(key_words_dict[tag])
                norm_tag_en_list.append(tag_dict[key_words_dict[tag]])
                continue
        norm_tag = '|'.join(norm_tag_list)
        norm_tag_en = '|'.join(norm_tag_en_list)
        if norm_tag != '':
...
def update_db(datas):
    sql = 'update ' + REST_TABLE + ' set norm_tagid=%s,norm_tagid_en=%s where id=%s'
