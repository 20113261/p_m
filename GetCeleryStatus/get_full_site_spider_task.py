...
@schedule.scheduled_job('cron', second='*/120')
def get_status():
    ...
    # celery message bool
    celery_data = list(filter(lambda x: 'full_site_task' == x['name'], j_data))[0]
    message_count = celery_data['messages']

    # insert task into queue
    if int(message_count) <= 9000:
        os.system('sh /root/data/PycharmProjects/celery_using/init_full_site_parse.sh')
    else:
