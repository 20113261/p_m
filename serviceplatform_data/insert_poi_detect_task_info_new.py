...
def _get_per_table_task_info(table_name):
    ...
    sql = '''SELECT
          ...
          FROM {2}
            JOIN {0}
              ON {2}.source = {0}.source AND
                 {2}.sid = {0}.source_id
          WHERE city_id != 'NULL' and {2}.`use` != '0'
          LIMIT {1},999999999;'''.format(table_name, offset, table_name.replace('list_', 'images_'))
    data = []
    _count = 0
...
def get_task_info():
    ...
    list_tables = list(
        sorted(
            filter(lambda x: x.startswith('list_total_qyer'),
                   map(lambda x: x[0],
